...
验证：登录页面中，输入已注册且与认证身份匹配的普通成员邮箱，以及满足密码规则且与该邮箱匹配的密码后，登录成功并进入 ORF 首页（悬赏大厅）。
...
- Setup：构造 S0。
  - [Setup-1] [api] 准备邮箱为 `orf-member-login-e2e@orf.local`、使用满足规则且与该邮箱匹配的固定测试密码的普通成员认证身份。
  - [Setup-2] [prisma] 准备邮箱为 `orf-member-login-e2e@orf.local`、角色为 `member`、状态为 `active` 的普通成员用户。
  - [Setup-3] [api] 设置普通成员默认进入页面为 ORF 首页（悬赏大厅）。
...
- S0：Action 前状态。
  ...
  - [S0-10] [api] 认证系统中 应存在 邮箱为 `orf-member-login-e2e@orf.local` 的普通成员认证身份。
  - [S0-11] [api] 邮箱为 `orf-member-login-e2e@orf.local` 的普通成员认证身份的满足规则且匹配的密码凭据 应可用。
  - [S0-12] [prisma] ORF 业务系统中 应存在 邮箱为 `orf-member-login-e2e@orf.local`、角色为 `member`、状态为 `active` 的普通成员用户。

- Action：被测业务动作。
  - [Action-1] [playwright] 在邮箱输入框输入 `orf-member-login-e2e@orf.local`。
  - [Action-2] [playwright] 在密码输入框输入满足规则且与该邮箱匹配的普通成员固定测试密码。
