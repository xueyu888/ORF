...
悬赏大厅只向普通成员展示尚未被任何成员正式接受的悬赏目标。申请被指挥官确认，或征召被成员接受后，目标进入重估并出现在我的挑战页面；指挥官/管理员不参与目标挑战。
...
点击目标行不打开详情弹窗。普通成员点击 `申请挑战` 或 `接受挑战` 打开确认弹窗。
...
| 重估中 | 显示在我的挑战；指挥官可新增和编辑指标，挑战者可提出和编辑指标；已有至少一个指标时指挥官可冻结 |
