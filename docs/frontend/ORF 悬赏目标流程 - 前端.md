...
| 管理动作 | 指挥官在挑战页内新建并编辑候选目标；发布后进入悬赏大厅；审核申请、重估完成后冻结、冻结后重开重估审批也在挑战页处理 |
...
| 已冻结 | 继续显示在悬赏大厅；不再开放公开申请或征召接受；指挥官可在挑战工作台加派 active 普通成员为正式挑战者，正式挑战者可在提交战利品前申请冻结后重开重估，或在我的挑战里提交战利品 |
| 待验收 | 继续显示在悬赏大厅；不再开放加入，也不再开放重开重估或难度修改；指挥官在挑战页验收 |
| 已验收 | 继续显示在悬赏大厅的 `待结算` 分组；不再开放加入，等待最终贡献比例和积分结算 |
| 已结算 | 继续显示在悬赏大厅，展示结算后的公开结果入口和参与状态 |

`flowStatus` 是前端状态展示的主字段。`stage` 只用于兼容页面阶段样式。

悬赏大厅申请入口、挑战页申请审核入口、冻结样式、冻结后重开重估申请和加派入口必须按统一 `flowStatus` 生命周期判断；冻结后残留的 pending 申请不展示通过或拒绝操作。冻结后重开重估只在挑战页通过 `frozenReestimate` 对齐申请处理，指挥官审批时设置新的重估截止时间，不能超过目标验收截止时间；该入口只存在于 `frozen`，目标进入 `submitted` 后不再出现。加派入口只属于挑战工作台的指挥官执行期调度，不在悬赏大厅开放。
...
| `POST /api/objectives/:objectiveId/alignment-requests` | 挑战者申请完成重估、冻结后重开重估或验收对齐；冻结后重开重估只允许 `frozen` 目标，待验收目标只能申请验收对齐 |
| `PATCH /api/objectives/:objectiveId/alignment-requests/:alignmentRequestId` | 指挥官处理对齐申请；冻结后重开重估审批通过时传新的 `confirmationDueAt` |
