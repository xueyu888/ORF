...
| 悬赏大厅 E2E | `e2e/bounties/bounty-hall-skin.spec.ts` / `e2e/challenges/orf-frontend-flow.spec.ts` | 悬赏大厅全员可见、指挥官完整界面但挑战动作阻断、征召置顶、申请后刷新和禁用重复申请 |
...
| 页面 | 定位 | 主体内容 | 不应该承担 |
| --- | --- | --- | --- |
| `/bounties` | 悬赏大厅 | 所有已通过用户可见 `open/applying/recruiting` 悬赏目标；active 普通成员申请挑战、接受自己的征召、查看已申请状态；指挥官/管理员看到完整大厅界面和操作区，但挑战动作弹窗阻断 | 不编辑目标、不编辑指标、不维护任务、不提交战利品；指挥官/管理员不能让申请或接受生效 |
...
- 所有已通过用户都能看到 `open/applying/recruiting` 大厅目标；角色不能导致大厅列表整体变空，也不能导致指挥官大厅操作区被隐藏。
- `open` 悬赏目标对 active 普通成员点击“申请挑战”进入正常确认；对指挥官/管理员点击同一入口时弹窗提醒“指挥官不应该申请挑战”，不能发起成功 mutation，也不能本地显示已申请。
...
| 系统消息 | `e2e/notifications/notification-center.spec.ts` 验证顶部栏消息铃铛、未读数、消息浮层、完整消息页、已读动作；消息不进入侧边栏主导航 |
...
- 新建目标树内 temporary 目标，以及指标、反馈、行动项和指标更新弹窗，不能预填演示业务文案；只能保留周期、截止日期、当前处理人等真实上下文默认值。
