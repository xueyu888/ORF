...
- [TestD 结果通知集成](backend/TestD结果通知集成.md)：测试计划终态、认证协议、公开频道投递与 MR 失败私聊。
...
| [ORF 反馈中心系统性重建设计.md](./rules/ORF%20反馈中心系统性重建设计.md) | 反馈中心目标状态机、模块边界、数据契约、导入导出、UI 和一次性迁移方案 |
...
| [聊天投票规则.md](./rules/聊天投票规则.md) | 聊天投票的两阶段状态、事实源、匿名边界、接口、界面和验证契约 |
