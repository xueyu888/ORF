...
modules/feedback/
  ...
  src/
    ...
    infrastructure/
      ...
      outbox/               # 可靠事件投递
      transfer/             # 当前阶段 CSV 导入导出；完整备份恢复后续单独实现
...
2026-08-08 阶段决策：当前可用版本只要求“当前视图 CSV 导出”完成真实闭环。完整备份 ZIP 属于后续迁移和恢复能力，可以暂缓，不阻塞当前反馈中心上线、Win11 客户端可用性和本阶段验收。

| 导出 | 用途 | 内容 | 当前阶段要求 |
| --- | --- | --- | --- |
| 当前视图 CSV | 分析、审计、轻量迁移 | 当前筛选下的核心字段、状态、结论、影响、优先级、处理人、项目、分类、时间和关系摘要 | 必须真实可用并可截图/下载验证 |
| 完整备份 ZIP | ORF 迁移和恢复 | `manifest.json`、版本化 JSONL、原始报告、讨论评论、两类附件及哈希、活动、关系、用户/项目引用映射 | 暂缓；后续恢复能力完成并验收后再进入正式完成范围 |
...
4. 当前运行契约只接受 CSV；ZIP、manifest、JSONL 和附件哈希校验属于后续完整备份恢复能力，未进入当前入口、路由和测试范围。
5. 导入不创建普通反馈通知，不恢复通知收据、阅读游标或订阅偏好。
6. 导入不会逐条调用普通创建接口；使用专用批量应用命令，避免 222 条历史反馈产生 222 次通知和刷新。
7. 更新已存在记录必须展示字段差异并再次确认；默认只新增，不覆盖。
8. 当前 CSV 导入不接收附件；后续完整备份恢复进入范围时，附件必须先进入批次暂存区，数据库事务失败后由批次清理器删除，成功后只有数据库已引用的对象可以保留。
9. 当前团队中对反馈可见的 active 成员均可导出可见数据并向当前团队导入；不再区分管理员和普通成员，但不得跨团队或跳过预检。

### 8.3 UI

- 顶部更多菜单当前阶段至少提供“导出当前视图 CSV”和“导入反馈”入口。
- 完整备份 ZIP 不属于当前运行契约，入口不展示；后续恢复能力、附件一致性和真实下载/恢复验收完成后，再作为独立阶段加入。
- 普通 CSV 必须经过字段映射和预览，不能为了“一键”跳过安全检查。
...
- 生命周期和处理人使用评论编辑器底栏、发送按钮左侧的两个紧凑菜单；默认不占独立内容行，只有选择复杂生命周期时才展开结论或代操作原因。侧栏只展示当前值，不再提供独立提交按钮；评论、状态和处理人仍由同一个发送按钮提交。只要本次跟进已经选择了生命周期命令或有效处理人变更，底栏和发送按钮必须保持可见，不能因为编辑器失焦或评论正文为空而隐藏；发送按钮是否可用只由评论草稿、跟进命令和上传状态决定。
...
新建反馈正文是报告编辑场景。输入区随内容增高，删除内容后回缩；达到视口约束的上限后才内部滚动。桌面初始正文高度为 `min(240px, 40dvh)`，720px 及以下视口为 `min(160px, 40dvh)`，增高上限统一为 `min(640px, 60dvh)`。上下限一起受动态视口约束，小屏和横屏不依赖固定的大高度；页面仍可滚动到附件、属性和创建按钮。

尺寸能力沿现有公开接口传递，不新增编辑器或前后端高度状态：

```mermaid
flowchart LR
  Page[新建反馈页：草稿与提交编排] -->|autoGrow| Host[反馈宿主适配与 CommentDraftFields：透传编辑能力]
  Host -->|autoGrow| Editor[富文本编辑器：测量内容、增高与回缩]
  Style[反馈样式：正文尺寸策略] -->|公开的最小与最大高度变量| Editor
  Page -->|description 与附件| API[反馈后端：校验、报告与附件持久化]
  API -->|成功返回反馈 ID；失败返回错误| Page
```

- `OrfRichTextEditor` 拥有内容测量和尺寸更新；正文变化、初次恢复草稿和容器宽度变化使用既有 `autoGrow` 逻辑。通用样式公开 `--orf-rich-text-editor-min-height`、`--orf-rich-text-editor-max-height`，由反馈自己的样式设置场景值，页面不穿透编辑器内部元素设置高度。
- `FeedbackWebHost.components.CommentDraftFields` 与宿主适配层只传递可选 `autoGrow`。新建反馈显式启用，未传参的评论、回复和原位编辑保留各自既有行为；新建正文的尺寸策略不泄漏到其它场景。
- 前端状态链为“编辑或恢复草稿 → 提交中 → 创建成功后清理草稿并进入详情”；提交失败保留草稿。草稿正文与附件引用是待提交内容，序列化生成完整 `description`；显示行数、测量高度和滚动位置均为临时展示结果，不写入本地草稿或请求，不截取可见部分作为正文。
- 后端沿既有 `POST /api/feedback` 契约校验团队、处理人、项目和附件，准备报告附件，并在反馈事务中写入 `feedback.description`、报告附件引用、创建活动及通知 outbox；提交前失败清理已准备的附件。报告及附件所有权仍按 6.8 节执行。尺寸变化不增加后端字段、迁移、接口参数或另一份正文。
...
- ZIP manifest、JSONL、附件数量和哈希一致是后续完整备份恢复能力的验收项；当前阶段不得保留可点击 ZIP 入口或运行时 ZIP 预检路径。
...
- 新建反馈正文在桌面与移动端验证空白初始空间、多行输入、长段落自动换行、粘贴、草稿恢复、删除回缩、视口宽高变化与达到上限后的滚动；光标、附件和创建按钮可访问，正文不丢失。提交失败保留草稿，成功才清理；高度不得进入草稿或提交数据。普通评论和详情跟进底栏保持既有交互。
...
- 用真实用户动作截图验证：处理人提交验证、发起人确认关闭、发起人退回、管理员接管确认/退回/重开、列表未读清除、重复关系、CSV 导出和导入预检。ZIP 进入正式范围时再单独补充真实导出/恢复验收。
...
7. 完成桌面端和移动端 UI 重建。
8. 完成 CSV、导入预检和幂等批次；ZIP 备份恢复作为后续迁移能力暂缓。
