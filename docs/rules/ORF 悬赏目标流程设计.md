...
- 新建目标属于挑战页内的候选目标编辑流程；全局入口只负责把用户带到挑战页。页面先插入完整目标草稿面板，标题输入按 Enter 或输入框失焦快速创建 `candidate`；请求发起后草稿立即退出标题编辑态并留在原位，API 返回真实目标后连续替换草稿并沿用草稿位置，任务管理数据刷新前不能出现目标消失或跳位。
