...
工作日志记录“某个用户在某一天每次提交了什么”。普通非 FAE 成员的日志默认必须绑定到目标，并填写目标进度估计和正文；默认目标候选只显示自己参与且尚未完成的目标，目标搜索可以查到当前团队全部目标。已验收、已结算、已关闭或自己不是挑战者的目标不进入默认候选，但可以通过目标搜索显式选中；前端需要提示这类选择不会改变目标状态，并在自己不是挑战者时提示确认目标选择。`请假` 是内置特殊分类，所有 active 成员都可以选择，且不要求目标或目标进度估计。指挥官/管理员可以记录可填写目标日志、管理分类日志或临时未归类日志；`管理事务` 仍属于指挥官/管理员的管理分类能力，普通成员不能选择。当前临时允许 FAE 成员邓滨虎、何永杰保存未归类日志，但不开放管理分类能力。它是独立事实源，不属于评论、任务进度、战利品、验收或积分。
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| `GET` | `/api/work-logs/objectives?mode=default\|search&q=关键词` | 当前用户可填写日志的目标，并带上当前用户对每个目标最近一次填写的进度估计兼容剩余值；`default` 只返回自己参与且尚未完成的目标，`search` 按关键词返回当前团队全部目标；所有可写用户返回内置 `请假` 分类，管理员额外返回管理分类和智能分类开关 |
