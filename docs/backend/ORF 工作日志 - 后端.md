...
工作日志记录“某个用户在某一天每次提交了什么”。普通成员的日志必须绑定到自己参与的目标；指挥官/管理员可以记录目标日志、独立分类日志或临时未归类日志。它是独立事实源，不属于评论、任务进度、战利品、验收或积分。
...
`objectiveId`、`categoryId`、`categoryName` 只能三选一。普通成员必须提供 `objectiveId`；管理员可以不提供，也可以提供现有分类 `categoryId` 或新分类 `categoryName`。`remainingEstimatePercent` 可为空；未指定目标时后端会归空。`durationMinutes` 可为空；填写时必须是 `1..1440` 的整数。
...
- 普通成员不能保存未归类日志或分类日志；找不到对应目标时，应先和指挥官确认目标关系。
- 任何人都不能代别人填写工作日志。
