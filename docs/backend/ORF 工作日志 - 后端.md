...
工作日志记录“某个用户在某一天每次提交了什么”。普通成员的日志默认必须绑定到自己参与的目标；指挥官/管理员可以记录目标日志、独立分类日志或临时未归类日志。当前临时允许 FAE 成员邓滨虎、何永杰保存未归类日志，但不开放日志分类能力。它是独立事实源，不属于评论、任务进度、战利品、验收或积分。
...
| `remaining_estimate_percent` | 这条日志里作者对目标剩余比例的主观估计，`0..100`；未填写或未指定目标时为空 |
...
同一用户同一天可以提交多条工作日志，也可以多次记录同一个目标或分类。目标/分类改名或删除后，历史日志继续显示快照；编辑已有日志时，如果目标/分类没有变化，只修改正文、目标剩余估计、记录时间并保留原快照。
...
`objectiveId`、`categoryId`、`categoryName` 只能三选一。普通成员默认必须提供 `objectiveId`；管理员和当前临时 FAE 例外成员可以不提供目标。只有管理员可以提供现有分类 `categoryId` 或新分类 `categoryName`。`remainingEstimatePercent` 可为空；未指定目标时后端会归空。`durationMinutes` 可为空；填写时必须是 `1..1440` 的整数。
...
- 当前临时 FAE 例外成员邓滨虎、何永杰可以保存自己的未归类日志；若指定目标，仍必须属于自己可写的目标。
...
- 单元格统计日志条数、覆盖归类数、最新剩余估计、记录时间、归类摘要和该单元内日志明细快照。
- 汇总统计日志总数、有记录日期、覆盖归类数、记录时间和平均剩余估计。

## LLM 分类推荐策略

智能分类是工作日志的辅助读服务，不是事实源。它只负责把一段日志正文临时推荐到“目标、已有日志分类、新日志分类、未归类”之一，不能创建目标、不能创建分类、不能保存日志，也不能更新目标进度、验收、积分或结算。
...
- 推荐接口只接收 `bodyMarkdown`。目标候选和分类候选由后端按当前默认作用域重新读取，调用方不能提交自定义候选集。
...
满足条件时创建 `worklog.reminder` 系统通知事件和接收人收据，再投影到聊天里的 `我的系统通知`，并通过 SSE 向这些用户发送 `workLogReminder` sticky 横幅。提醒不创建日志、不改变目标、不影响进度、验收或积分。
