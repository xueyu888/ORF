...
工作日志记录“某个用户在某一天每次提交了什么”。普通非 FAE 成员的日志默认必须绑定到自己参与且尚未验收完成的目标，或日志日期等于验收/结算当天的已完成目标，并填写目标进度估计和正文；指挥官/管理员可以记录可填写目标日志、独立分类日志或临时未归类日志。当前临时允许 FAE 成员邓滨虎、何永杰保存未归类日志，但不开放日志分类能力。它是独立事实源，不属于评论、任务进度、战利品、验收或积分。
...
同一用户同一天可以提交多条工作日志，也可以多次记录同一个目标或分类。目标/分类改名、删除或进入已验收/已结算/已关闭后，历史日志继续显示快照；编辑已有日志时，如果目标/分类没有变化，只修改正文、目标进度估计、记录时间并保留原快照。已验收目标只在 `work_date` 等于 `Objective.acceptedAt` 所在自然日时继续进入目标候选，用于补写验收当天日志；已结算目标只在 `work_date` 等于最终结算事件 `objective_settlement_events.created_at` 所在自然日时继续进入目标候选，用于补写结算当天日志；已关闭目标不再进入新日志候选。
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| `GET` | `/api/work-logs/objectives?date=YYYY-MM-DD` | 当前用户在指定日志日期可填写日志的目标，并带上当前用户对每个目标最近一次填写的进度估计兼容剩余值；管理员额外返回分类和智能分类开关 |
...
- active 普通非 FAE 成员可以写自己的工作日志，且 `objectiveId` 必须属于当前默认作用域、包含当前用户 `users.id`，并且目标未进入 `closed` 状态；`accepted` 目标只有在日志日期等于 `Objective.acceptedAt` 所在自然日时仍可填写，`settled` 目标只有在日志日期等于最终结算事件 `objective_settlement_events.created_at` 所在自然日时仍可填写；`remainingEstimatePercent` 必须随目标日志一起提交。
- active 指挥官/管理员可以写自己的工作日志；`objectiveId` 可以为空。若指定目标，目标必须属于当前默认作用域，并且满足同一套日志日期目标候选规则。
...
- 推荐接口只接收 `bodyMarkdown` 和可选 `workDate`。目标候选和分类候选由后端按当前默认作用域和日志日期重新读取，调用方不能提交自定义候选集。
