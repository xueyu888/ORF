...
工作日志记录“某个用户在某一天每次提交了什么”。普通非 FAE 成员的日志默认必须绑定到目标，并填写目标进度估计和正文；普通成员的默认目标候选只显示自己参与且尚未完成的目标，目标搜索可以查到当前团队全部目标。已验收、已结算、已关闭或自己不是挑战者的目标不进入普通成员默认候选，但可以通过目标搜索显式选中；前端需要提示这类选择不会改变目标状态，并在自己不是挑战者时提示确认目标选择。指挥官/管理员的默认目标候选就是当前团队全部可作为日志记录对象的目标，包括已验收、已结算、已关闭以及自己不是挑战者的目标。`请假` 是内置特殊分类，所有 active 成员都可以选择，且不要求目标或目标进度估计。指挥官/管理员可以记录可填写目标日志、管理分类日志或临时未归类日志；`管理事务` 仍属于指挥官/管理员的管理分类能力，普通成员不能选择。当前临时允许 FAE 成员邓滨虎、何永杰保存未归类日志，但不开放管理分类能力。它是独立事实源，不属于评论、任务进度、战利品、验收或积分。
...
新增日志保存成功后会创建 `worklog.submitted` 系统公告，通知当前团队 active 成员有人提交了某天工作日志。公告正文只包含提交人和日期，目标/分类标题保存在 `metadata` 里供卡片兜底展示；通知携带跳转到对应日志条目的链接 `/work-logs?date={date}&view=today&entry={entryId}`，不保存完整日志正文；编辑和删除历史日志不发送这类公告。通知事实源仍是 `notification_events`、`notification_receipts` 和 `notification_deliveries`，不能反向定义日志正文、报表或欠账状态。
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| `GET` | `/api/work-logs/objectives?mode=default\|search&q=关键词` | 当前用户可填写日志的目标，并带上当前用户对每个目标最近一次填写的进度估计兼容剩余值；`default` 对普通成员只返回自己参与且尚未完成的目标，对指挥官/管理员返回当前团队全部可挂载目标；`search` 按关键词返回当前团队全部目标；所有可写用户返回内置 `请假` 分类，管理员额外返回管理分类和智能分类开关 |
...
- active 普通非 FAE 成员可以写自己的工作日志。目标日志的 `objectiveId` 必须属于当前默认作用域；目标可以是尚未完成、已验收、已结算或已关闭状态，也可以不是当前用户挑战的目标，但这类目标只通过搜索候选显式暴露；`remainingEstimatePercent` 必须随目标日志一起提交。
- active 成员都可以选择内置 `请假` 分类保存自己的免目标日志；`请假` 不要求 `remainingEstimatePercent`，也不创建或修改 `work_log_categories`。
- active 指挥官/管理员可以写自己的工作日志；`objectiveId` 可以为空。若指定目标，目标必须属于当前默认作用域；默认目标候选、搜索候选和保存校验都允许当前团队全部可挂载目标。
- 当前临时 FAE 例外成员邓滨虎、何永杰可以保存自己的未归类日志；若指定目标，仍必须属于自己可写的目标，但不强制填写目标进度估计。
- 只有 active 指挥官/管理员可以使用或创建管理日志分类，也只有他们可以请求智能分类建议；`管理事务` 属于该能力范围。
- 除内置 `请假` 和当前临时 FAE 例外外，普通成员不能保存未归类日志或管理分类日志；找不到对应目标时，应先和指挥官确认目标关系。
- 任何人都不能代别人填写工作日志。
...
- 推荐接口只接收 `bodyMarkdown`。目标候选和分类候选由后端按当前默认作用域重新读取，且只使用当前用户的默认目标候选；指挥官/管理员默认候选是当前团队全部可挂载目标，普通成员默认候选仍是自己参与且尚未完成的目标。调用方不能提交自定义候选集，普通成员推荐不会自动命中搜索专用目标。
