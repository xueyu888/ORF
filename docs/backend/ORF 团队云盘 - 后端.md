...
- Slack 式群聊资源入口：`chat_channel_drive_links` 让频道默认以轻量资源流显示已绑定资源和默认上传文件夹；聊天右栏只保留状态标题、搜索命令栏、当前资源预览舞台和资源流。命令栏默认只暴露搜索和单一添加菜单，上传资源与打开完整资源库收在这个菜单里，面板仍支持拖拽上传；固定到频道、下载、打开详情、移出频道等次级操作只出现在当前预览资源的菜单里，版本、回收站、上下文管理等完整操作留在 `/resources`。
...
- Linear 式工作上下文：`drive_node_context_links` 把文件显式关联到项目、目标、指标、任务、反馈、工作日志或聊天上下文，但不改变这些业务对象或云盘自身事实源；目标、指标、任务、反馈和工作日志页面通过 `/api/drive/search?contextType=...&contextId=...` 读取相关资源投影，并通过云盘上传和 context-link API 编排上传、关联已有资源、取消关联，不在业务页复制云盘删除、恢复、版本或详情状态机。工作日志的当天资源块只是当前日期各条日志资源的派生汇总，不是新的日期级文件归属事实。
