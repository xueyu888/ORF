...
- Slack 式群聊资源入口：`chat_channel_drive_links` 让频道默认以轻量资源流显示已绑定资源、默认上传文件夹和行内资源预览；频道内搜索常驻在入口顶部，上传作为唯一主动作保留在顶部，打开完整资源库降为底部弱入口，面板仍支持拖拽上传；固定到频道、下载、打开详情、移出频道等次级操作收在单个资源行菜单里，版本、回收站、上下文管理等完整操作留在 `/resources`。
