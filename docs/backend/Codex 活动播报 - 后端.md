...
播报不是会话转发器：原始用户消息和 Codex 回复只作为分类信号使用，输出前会改写成简明活动总结，再套用当前轮换风格。消息正文用清楚、自信的龙傲天式语气概括本轮问答内容，但中文主文案不用“标题：正文”的小标题格式，并压成一句简短、有气势的话；英文行也用一句话覆盖中文主文案，并保留关键英文单词音标和语法说明；部分风格可以追加本地文字表情包 cue。
