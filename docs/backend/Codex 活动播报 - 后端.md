...
播报不是会话转发器：原始用户消息和 Codex 回复只作为分类信号使用，输出前会改写成简明活动总结，再套用当前轮换风格。消息正文用清楚、自信的龙傲天式语气概括本轮问答内容，但中文主文案不用“标题：正文”的小标题格式；英文行覆盖整段中文主文案，按句对应翻译，并保留关键英文单词音标和语法说明；部分风格可以追加本地文字表情包 cue。
