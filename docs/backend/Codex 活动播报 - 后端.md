...
播报不是会话转发器：原始用户消息和 Codex 回复只作为分类信号使用，输出前会改写成简明活动总结，再套用当前轮换风格。每条消息开头用 Markdown 分隔线 `---` 拉开边界，方便在 Mattermost 里区分轮次。消息正文要先说本轮任务，再说具体动作，最后点明结果或影响；中文主文案不用“标题：正文”的小标题格式，并压成一句简短、清晰、有气势的话，不能只写“问答清楚”“气势拉满”这类空泛判断；英文行也用一句话覆盖中文主文案，并保留关键英文单词音标和语法说明；部分风格可以追加本地文字表情包 cue。
