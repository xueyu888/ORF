...
在局域网环境中，由 ORF 后端主动拉取 GitHub 上 `xueyu888/ORF` 仓库的最新提交，把新增提交摘要转发到 Mattermost 的 `LLM / ORF` 频道。
...
| `GITHUB_REPOSITORY_FULL_NAME` | 允许同步的仓库全名，当前为 `xueyu888/ORF`。 |
| `GITHUB_SYNC_ENABLED` | 是否启用局域网轮询同步，当前设为 `true`。 |
| `GITHUB_SYNC_BRANCH` | 轮询分支，当前为 `main`。 |
...
2. 每隔 `GITHUB_SYNC_INTERVAL_SECONDS` 请求 GitHub commits API。
3. 如果发现 `main` 分支有新提交，把新增提交列表发到 Mattermost ORF 频道。
