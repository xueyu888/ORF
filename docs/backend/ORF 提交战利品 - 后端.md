...
`memberUserId` 对应 `users.id`，是贡献分配的身份事实源；`member` 是展示名兼容字段。`ratio` 是共享结算服务和 ORF 结算接口使用的目标级标准比例，范围为 `0..1`。一份评分必须覆盖当前目标的全部普通成员挑战者，不能重复成员，且 `ratio` 合计必须为 `1`。前端页面可以按指标行填写 `0..100` 的贡献百分比，但指标级输入和本地草稿只属于浏览器端录入和计算状态；前端按指标权重汇总后，仍只把目标级 `allocations` 转成标准比例并加密提交。弃权必须带非空 `abstentionReason`，不带 `allocations` 参与均值计算。
