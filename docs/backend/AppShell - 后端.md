...
当前用户个人偏好在前端 API 层按 `userId` 做单飞缓存；AppShell、通知 Toast、个人设置页和登录落地页共享同一个读取入口，保存成功后以返回值更新缓存。设置失效事件或登录成功落地页读取可以显式强制刷新，不能让每个组件各自轮询 `/api/settings/personal/preferences`。
