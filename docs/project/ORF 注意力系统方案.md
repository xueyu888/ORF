...
10. 聊天和有用户触发人的系统 Toast 展示发送者头像；Windows 使用临时本地 PNG 生成圆形 `appLogoOverride`，其他桌面平台使用同一 PNG 作为通知 icon。Windows 聊天实时 Toast 使用系统 `short` 时长，普通注意力 Toast 保持系统默认时长；Android 继续使用应用通知图标，因为现有本地通知插件的 `largeIcon` 契约只接受 Android drawable 资源，不另建一套头像缓存和下载链。
