...
6. “已读”和“已处理”必须分离。注意力系统最多改变已读或本地稍后提醒状态，不得直接把反馈、目标、工作日志等业务对象标记为已处理。
...
业务 mutation 成功
  ...
  -> Win11 桌面壳展示任务栏角标、系统 Toast、任务栏闪烁和托盘菜单
```
...
| `flash` | `toast` + Win11 任务栏图标闪烁；仅在窗口不在前台时触发 | `comment.mention.created`、聊天 `@我`、私聊、我关注的话题有回复 |
| `urgent` | `toast` + 任务栏闪烁 + 托盘置顶入口 + 侧边栏“待我处理”置顶 | `feedback.assigned`、`objective.recruitment.created`、`objective.reinforcement.added`、`objective.alignment.requested`、`objective.loot.submitted`、`objective.peerReview.requested`、`objective.revision.required`、active `WorkLogReminderState`、`data.sync.conflict` |
...
| 标记已读 | 调用现有通知/聊天已读能力，只改变注意力状态 | 可后续实现 |
...
| 等级 | 任务栏 overlay | Windows 系统 Toast | 任务栏 flash | 托盘菜单 |
| --- | --- | --- | --- | --- |
| `none` | 清空 | 不展示 | 停止 | 普通菜单 |
| `badge` | 显示数字/红点 | 不展示 | 不触发 | 打开待处理提醒、打开聊天、打开我的系统通知 |
| `toast` | 显示数字/红点 | 展示一次 | 不触发 | 打开聊天/系统通知 |
| `flash` | 显示数字/红点 | 展示一次 | 窗口不在前台时触发 | 打开最新提醒 |
| `urgent` | 显示数字/红点 | 展示一次 | 窗口不在前台时触发 | 置顶“打开待处理提醒” |
...
5. Win11 托盘菜单新增“打开待处理提醒”，点击后跳转到 `latestTargetPath`。
6. 侧边栏新增“待我处理”入口，只展示数量和轻量面板，点击进入现有聊天系统消息或业务页面。
7. 移动端底部导航新增“待办”入口，不新增独立页面；点击进入最新待处理目标或个人系统通知。

当前不做：

1. 不新增独立通知中心页面。
2. 不实现“标记已读”和“标记已处理”动作。
...
| 窗口失焦 | `flash` 或 `urgent` 触发任务栏闪烁 |
| 窗口最小化 | `flash` 或 `urgent` 触发系统 Toast 和任务栏闪烁 |
| 窗口隐藏到托盘 | 托盘菜单可以打开待处理提醒 |
