...
服务器本机和局域网成员位于同一个局域网内，当前路由器不支持稳定 NAT 回环；本机运行时依赖不要指向 `125.70.13.137`。当前 WSL mirrored 主机上的 ORF 后端通过 `DATABASE_URL=...@127.0.0.1:5432/...` 连接 Windows PostgreSQL，避免与容器出口端口转换发生冲突；Ory 容器通过显式 `ORY_DATABASE_URL=...@199.199.199.8:5432/...` 访问同一数据库，其宿主迁移探针通过 `ORY_DATABASE_PROBE_URL=...@127.0.0.1:5432/...` 访问。三者保留原数据库、schema、权限账号和完整 TLS 证书校验；证书必须包含各自连接地址。Ory Public 和 MinIO 的本机入口仍为 `127.0.0.1:4433` 和 `127.0.0.1:9000`。局域网成员接入共享环境时，PostgreSQL、Ory Public 和 MinIO 应使用 `199.199.199.8` 入口，由共享包安装脚本的 `--mode auto` 自动选择。
...
`dist/assets` 使用 hash 文件名和长期缓存。`npm run build` 先在隔离目录生成并校验当前构建，再发布“当前构建 + 上一构建”的 hash 资源，最后自动删除更旧文件；新入口写入前会先准备全部新资源，因此一次发布窗口内仍打开旧页面的客户端可以继续加载，磁盘占用也保持有界。Vite 禁用 `publicDir`，后端设置样板和内置背景不会复制进 Web/APK；`.vite` 构建清单和 `/settings/{users,system,user}` 遗留私有路径由 public-gateway 明确拒绝访问。
