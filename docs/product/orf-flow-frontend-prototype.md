...
ORF Flow 的 Result 设计必须以 Notion 页面 `指标模板` 为核心思想来源。该模板强调：指标不是任务标题，而是可审计、可验证、可分发、可复盘的交付契约。

从模板沉淀出的产品原则：

1. 每个大指标必须有负责人、评级和 3-5 条具体指标。
2. 每条指标使用“指标名：要求”的格式，一条只说一件事。
3. 能量化必须量化；不能量化时，必须写清支持范围和完成状态。
4. 不写实现方案，不写“尽量、较好、完善、高效”等模糊词。
5. 涉及成功率、覆盖率、TopK、P95、时长时，必须说明统计对象和完成标准。
6. 标准样本集和标准问题集由负责人提前确认；标准问题集需要标注正确文本片段和期望答案。
7. TopK 指检索结果按相关性排序后的前 K 条文本片段；Top3 命中表示正确文本片段出现在前 3 条结果中。
8. P95 指同一测试环境下，95% 的请求或操作耗时不超过目标值。
9. 响应时间类指标必须说明测试数据规模、文件类型和测试环境。
10. 涉及模型生成的响应时间，必须固定模型、参数、上下文长度和输出长度；否则只统计检索、重排、引用组装等系统侧耗时。

评级标准来自该模板，评估的是 AI 编程下的交付不确定性，不按人工代码工作量估算：

- 普通：边界清楚，主要是单模块能力、常规管理能力或成熟开源组件的标准接入，AI 按文档实现后可直接检查。
- 复杂：涉及多模块协同、较大数据规模、性能口径、完整链路打通或开源组件深度接入。
- 攻坚：结果明显依赖样本、模型、算法、数据质量或第三方组件能力，需要多轮验证、调参或替换方案评估。
- 挑战：接近当前模型、算法、数据或开源组件能力上限，目标不一定能稳定达成，需要专项预研。

映射到 ORF Flow：

- `Objective` 是大指标或阶段性目标。
- `Result` 是具体指标契约，必须显示指标名、要求、统计对象、完成标准、样本集、测量范围、负责人和评级。
- `Task` 只是完成某个 Result 的执行动作，不能替代 Result。
- `Feedback` 是对 Result 口径、样本、完成状态或执行动作的修正信号。
...
- description。
- metricName。
- metricRequirement：使用“指标名：要求”的格式。
- statisticalObject：统计对象。
- completionStandard：完成标准。
- sampleSet：标准样本集或标准问题集。
- measurementScope：测试环境、数据规模、文件类型、模型参数、上下文长度等测量范围。
- deliveryRating：`普通` | `复杂` | `攻坚` | `挑战`，评估 AI 编程交付不确定性。
- baseline。
- current。
- target。
- unit。
