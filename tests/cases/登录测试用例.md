...
- Action：
  ...
  - [playwright] 按业务语义定位 `Password` 输入框，输入测试密码。
...
- S0 断言：
  ...
  - [playwright] `page.getByLabel("Password")` 可见且值为空。
...
- Clean 后断言：
  - [api] `/api/auth/session` 返回 `authenticated: false`。
  - [playwright] 浏览器上下文 cookies 中不存在 `orf_ory_session`。
  - [playwright] 访问受保护路由会回到 `/auth`。
  - [api] Ory/Kratos 测试身份仍然存在。
  - [prisma] ORF 测试用户仍然存在。
  - [prisma] 该用户的团队角色仍然是 `member`。
  - [prisma] 如恢复了 `last_login_at`，确认其等于 Setup 前记录的值。
