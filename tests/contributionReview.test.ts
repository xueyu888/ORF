...
test("contribution review summary ignores self score for settlement ratios", () => {
  const summary = summarizeContributionReviews(["Ada", "Bo"], [
    review("review-a", "Ada", [
  ...
  assert.equal(summary.ratios.find((item) => item.member === "Ada")?.ratio.toFixed(3), "0.778");
  assert.equal(summary.ratios.find((item) => item.member === "Bo")?.ratio.toFixed(3), "0.222");
});
...
function review(id: string, reviewer: string, allocations: ObjectiveContributionReview["allocations"]): ObjectiveContributionReview {
  return {
    id,
    objectiveId: "obj-review",
    reviewer,
    allocations,
    submittedAt: id.endsWith("a") ? "2026-05-29T00:00:01.000Z" : "2026-05-29T00:00:02.000Z",
  };
}
