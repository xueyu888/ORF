...
import { isReportsReadModelPath, taskManagementPathForRole } from "../src/state/orfDataLoading";
...
test("task management read model selection no longer depends on navigation path", () => {
  assert.equal(isReportsReadModelPath("/resources"), false);
  assert.equal(isReportsReadModelPath("/chat"), false);
  assert.equal(taskManagementPathForRole("admin"), "/api/tasks-page");
});
