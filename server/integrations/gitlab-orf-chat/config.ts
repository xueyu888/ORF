...
const configSchema = z.object({
  ...
  GITLAB_ORF_CHAT_SIGNING_TOKEN: optionalNonEmptyString,
  GITLAB_ORF_CHAT_WEBHOOK_MAX_BODY_BYTES: z.coerce.number().int().positive().default(1024 * 1024),
  GITLAB_ORF_CHAT_HOOK_MODE: z.enum(["group", "project", "both"]).default("group"),
...
export function gitLabOrfChatWebhookConfigured(config: GitLabOrfChatConfig) {
  return Boolean(config.GITLAB_ORF_CHAT_ENABLED && (config.GITLAB_ORF_CHAT_WEBHOOK_SECRET || config.GITLAB_ORF_CHAT_SIGNING_TOKEN));
}
...
export function gitLabOrfChatReconcilerConfigured(config: GitLabOrfChatConfig) {
  return Boolean(
    config.GITLAB_ORF_CHAT_ENABLED &&
      ...
      (config.GITLAB_ORF_CHAT_WEBHOOK_SECRET || config.GITLAB_ORF_CHAT_SIGNING_TOKEN),
  );
}
