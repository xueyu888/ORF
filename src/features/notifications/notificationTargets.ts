...
export function notificationTargetHref(notification: AppNotification) {
  ...
  if (
    notification.kind === "objective.published" ||
    notification.kind === "challenge.application.approved" ||
    notification.kind === "challenge.application.rejected" ||
    notification.kind === "objective.recruitment.created"
  ) {
