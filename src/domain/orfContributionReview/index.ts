...
export function summarizeContributionReviews(
  ...
  const members = uniqueParticipantNames(challengers);
  if (members.length === 0) {
    return {
      status: "missing",
      ratios: [],
  ...
  if (members.length === 1) {
    return {
      status: "ready",
      ratios: [{ member: members[0]!, ratio: 1 }],
      missingReviewers: [],
      reviewers: [],
    };
  }
  ...
  const missingReviewers = members.filter(
    (member) => !latestReviews.has(member),
  );
  ...
  const peerValuesByMember = members.map((member) => ({
    ...
    values: reviewEntries
      ...
      .map(
        (review) =>
          review.allocations.find((allocation) => allocation.member === member)
            ?.ratio ?? 0,
      ),
  }));
...
export function validateContributionAllocationInput(
  ...
  for (const allocation of allocations) {
    ...
    const key =
      allocationUserId ||
      targetByName.get(member)?.memberUserId?.trim() ||
      member;
    const ratio = Number(allocation.ratio);
    if (!targetByKey.has(key) || ratioByMember.has(key)) {
...
function uniqueContributionMemberTargets(
  values: Array<string | ContributionMemberTarget>,
) {
  const targets: ContributionMemberTarget[] = [];
  const seenKeys = new Set<string>();
  for (const value of values) {
    const member =
      typeof value === "string" ? value.trim() : value.member.trim();
    if (!member) continue;
    const memberUserId =
      typeof value === "string" ? null : value.memberUserId?.trim() || null;
...
function contributionMemberKey(target: ContributionMemberTarget) {
  return target.memberUserId?.trim() || target.member;
}
