...
export function ReportsPage() {
  ...
  return (
    <PageScaffold title="统计" subtitle="统计历史目标完成情况、团队成员积分和反馈趋势。" action={<div className="flex gap-2"><Button variant="secondary">演示模式</Button><Button onClick={() => notify("统计摘要已复制")}>复制摘要</Button></div>}>
      <Card className="orf-card-padding"><div className="text-sm font-semibold orf-text-primary">统计摘要</div><p className="mt-3 text-sm orf-text-secondary">ORF Flow 显示工程信心正在提升，但检索新鲜度、幻觉率和时延仍是当前 AI 应用周期的最高风险。</p></Card>
      ...
      <div className="grid gap-4 lg:grid-cols-2"><Card className="orf-card-padding"><div className="mb-3 text-sm font-semibold orf-text-primary">反馈主题</div><div className="flex flex-wrap gap-2">{state.causeCategories.slice(0, 8).map((cause) => <span key={cause} className="orf-status-tag border orf-border orf-surface-muted px-3 py-1 text-sm orf-text-secondary">{cause}</span>)}</div></Card><DecisionLog decisions={state.decisions} /></div>
      <Card className="orf-card-padding"><div className="text-sm font-semibold orf-text-primary">积分统计</div><div className="mt-3 grid gap-2 md:grid-cols-3">{["目标完成积分", "指标贡献拆分", "成员历史趋势"].map((item) => <div key={item} className="rounded-md orf-surface-muted p-3 text-sm orf-text-secondary">{item}</div>)}</div></Card>
    </PageScaffold>
  );
}
