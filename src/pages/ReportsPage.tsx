...
export function ReportsPage() {
  ...
  return (
    <PageScaffold title="汇报" subtitle="面向管理者查看目标进度、反馈主题和下周重点。" action={<div className="flex gap-2"><Button variant="secondary">演示模式</Button><Button onClick={() => notify("摘要已复制")}>复制摘要</Button></div>}>
      <Card className="orf-card-padding"><div className="text-sm font-semibold orf-text-primary">管理摘要</div><p className="mt-3 text-sm orf-text-secondary">ORF Flow 显示工程信心正在提升，但检索新鲜度、幻觉率和时延仍是当前 AI 应用周期的最高风险。</p></Card>
      ...
      <Card className="orf-card-padding"><div className="text-sm font-semibold orf-text-primary">下周重点</div><div className="mt-3 grid gap-2 md:grid-cols-3">{["版本感知检索", "Agent 幂等保护", "长上下文成本降低"].map((item) => <div key={item} className="rounded-md orf-surface-muted p-3 text-sm orf-text-secondary">{item}</div>)}</div></Card>
    </PageScaffold>
  );
}
