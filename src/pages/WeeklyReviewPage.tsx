...
function ReviewSection({ title, items }: { title: string; items: string[] }) {
  return <Card className="p-4"><div className="mb-3 text-sm font-semibold orf-text-primary">{title}</div><div className="grid gap-2">{items.map((item) => <div key={item} className="rounded-md orf-surface-muted p-3 text-sm orf-text-secondary">{item}</div>)}</div></Card>;
}
