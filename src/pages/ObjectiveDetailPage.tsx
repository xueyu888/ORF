...
export function ObjectiveDetailPage() {
  ...
  return (
    ...
    >
      ...
      {tab === "Overview" && (
        <div className="grid gap-4 xl:grid-cols-[1fr_320px]">
          ...
          <div className="grid content-start gap-4">
            ...
            <Card className="p-4"><div className="text-sm font-semibold orf-text-primary">相关 AI 系统</div><div className="mt-3 flex flex-wrap gap-2">{["RAG 服务", "Agent 运行时", "评估流水线", "权限审计"].map((item) => <span key={item} className="rounded-full border orf-border px-2 py-1 text-xs orf-text-secondary">{item}</span>)}</div></Card>
          </div>
        </div>
      )}
      ...
      {tab === "Feedback" && <Card className="grid gap-3 p-4">{feedback.map((item) => <div key={item.id} className="grid gap-2"><FeedbackCard feedback={item} resultTitle={results.find((result) => result.id === item.linkedResultId)?.title} /><select className="orf-input max-w-48 px-2 py-1 text-xs" value={item.status} onChange={(event) => updateFeedbackStatus(item.id, event.target.value as FeedbackStatus)}>{(["New", "Reviewing", "Action Created", "Result Updated", "Closed"] as FeedbackStatus[]).map((status) => <option key={status} value={status}>{feedbackStatusLabel[status]}</option>)}</select></div>)}</Card>}
