...
export function AIEvaluationPage() {
  ...
  return (
    <PageScaffold title="AI 评估" subtitle="追踪 AI 应用流程的质量、成本、时延和可靠性。">
      <div className="grid gap-4 md:grid-cols-3 xl:grid-cols-6">{metrics.map(([label, value]) => <Card key={label} className="p-4"><div className="text-xs orf-text-muted">{label}</div><div className="mt-2 text-2xl font-semibold orf-text-primary">{value}</div></Card>)}</div>
