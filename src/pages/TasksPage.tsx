...
type CommentDraftMode =
  | { type: "default" }
  | { type: "reply"; rootMessageId: string; targetMessageId: string; targetAuthor: string }
  | { type: "edit"; threadId: string; messageId: string; targetAuthor: string };
...
function CommentPanel({
  ...
  const commentEntries = useMemo<CommentEntry[]>(() => {
    ...
    return entries.map((entry) => ({
      ...entry,
      replyCount: replyCounts.get(entry.message.id) ?? 0,
    }));
  }, [threads]);
  ...
  const repliesByRootId = useMemo(() => {
    ...
    for (const [rootMessageId, entries] of groups) {
      groups.set(
        rootMessageId,
        entries.sort((left, right) => left.message.createdAt.localeCompare(right.message.createdAt)),
      );
    }
  ...
  const handleDelete = (threadId: string, messageId: string) => {
    ...
    if (deletingRoot && activeRootMessageId === messageId) {
      setActiveRootMessageId(null);
    }
          ...
          {activeRootEntry ? (
            <div className="orf-comment-reply-detail-view">
              <div className="orf-comment-detail-header">
                <button type="button" className="orf-comment-icon-button" aria-label="返回外层评论列表" title="返回" onClick={closeReplyDetail}>
      ...
      <div className="orf-comment-message-main">
        <div className="orf-comment-message-header">
          ...
          <div className="orf-comment-meta">
            ...
            <button
              type="button"
              className="orf-comment-icon-button orf-comment-icon-button-danger"
              aria-label="删除评论"
              title="删除"
              onClick={(event) => {
                event.stopPropagation();
        ...
        {showReplyEntry && (
          <button
            type="button"
            className="orf-comment-reply-count"
            onClick={(event) => {
              event.stopPropagation();
              onEnterReplies?.();
            }}
