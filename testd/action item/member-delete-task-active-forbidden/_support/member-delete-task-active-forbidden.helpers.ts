...
} from "./member-delete-task-active-forbidden.context";
...
export async function prepareForbiddenActiveDeleteTaskTarget(
  target: MemberDeleteTaskActiveForbiddenTarget,
...
export async function targetFlowStatus(target: MemberDeleteTaskActiveForbiddenTarget) {
  return (await readObjective(target.objective.id))?.flowStatus ?? null;
}

export async function targetStage(target: MemberDeleteTaskActiveForbiddenTarget): Promise<OrfStage | null> {
...
export function taskDeleteMenuButton(
  page: Page,
  target: MemberDeleteTaskActiveForbiddenTarget,
  task: Pick<MemberDeleteTaskActiveForbiddenFixture, "title">,
) {
  return targetTaskRow(page, target, task).locator('button[aria-label*="打开块菜单"]').first();
}

export async function memberWorkbenchMissingObjective(page: Page, target: MemberDeleteTaskActiveForbiddenTarget) {
...
export async function memberWorkbenchMissingTask(page: Page, task: Pick<MemberDeleteTaskActiveForbiddenFixture, "id" | "title">) {
  const response = await readMemberWorkbenchData(page);
  if (response.status !== 200) return false;
