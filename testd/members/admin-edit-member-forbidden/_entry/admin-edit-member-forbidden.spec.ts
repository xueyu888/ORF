...
import { adminEditMemberForbiddenCase } from "../admin-edit-member-forbidden.case";

test.describe("管理员编辑成员反向测试用例", () => {
  test(adminEditMemberForbiddenCase.title, async ({ context, page }, testInfo) => {
    const ctx: MemberManagementForbiddenTestContext = { context, page };

    await runStateCase(adminEditMemberForbiddenCase, ctx, {
