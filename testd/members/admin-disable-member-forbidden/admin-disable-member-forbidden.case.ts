...
export const adminDisableMemberForbiddenCase = {
  id: "members.admin-disable.member-forbidden",
  title: "管理员停用成员-普通成员不可停用成员",
  model: STATE_CASE_MODEL,
  tags: ["members", "disable", "member", "permission", "negative-path"],

  data: {
    memberEmail: "orf-member-disable-forbidden-e2e@orf.local",
    memberPassword: "OrfMemberDisableForbiddenE2E!2026",
    memberName: "ORF Member Disable Forbidden E2E",
    memberRole: "member",
    targetUserId: "user-member-disable-forbidden-target-e2e",
    targetName: "ORF Member Disable Forbidden Target E2E",
    targetEmail: "orf-member-disable-forbidden-target-e2e@orf.local",
    targetRole: "member",
    targetStatus: "active",
  },
  ...
  Setup: {
    ...
    steps: [
      ...
      { source: { caseStepId: "Setup-2", method: "api" }, id: "ory.member_identity.upsert", title: "准备普通成员认证身份，邮箱为 `orf-member-disable-forbidden-e2e@orf.local`，密码为固定测试密码", object: "ory.identity", operator: "upsert_password", params: { emailFrom: "data.memberEmail", nameFrom: "data.memberName", passwordFrom: "data.memberPassword", saveAs: "memberIdentity" } },
      { source: { caseStepId: "Setup-3", method: "prisma" }, id: "db.member.upsert", title: "准备普通成员用户和默认团队成员关系，邮箱为 `orf-member-disable-forbidden-e2e@orf.local`、角色为 `member`、状态为 `active`", object: "db.user", operator: "upsert", params: { emailFrom: "data.memberEmail", nameFrom: "data.memberName", roleFrom: "data.memberRole", status: "active", identityIdFrom: "runtime.memberIdentity.id", saveAs: "memberUser" } },
      { source: { caseStepId: "Setup-4", method: "prisma" }, id: "db.target.upsert", title: "准备被管理成员，邮箱为 `orf-member-disable-forbidden-target-e2e@orf.local`、角色为 `member`、状态为 `active`", object: "db.user", operator: "upsert", params: { userIdFrom: "data.targetUserId", emailFrom: "data.targetEmail", nameFrom: "data.targetName", roleFrom: "data.targetRole", statusFrom: "data.targetStatus", saveAs: "targetUser" } },
  ...
  S0: {
    description: "普通成员已登录，被管理成员处于启用状态",
    assertions: [
      { source: { caseStepId: "S0-1", method: "api" }, id: "session.member.authenticated", title: "当前会话 应为 邮箱为 `orf-member-disable-forbidden-e2e@orf.local`、角色为 `member`、状态为 `active` 的已登录会话", object: "auth.session", operator: "authenticated", params: { emailFrom: "data.memberEmail", roleFrom: "data.memberRole", status: "active" } },
      ...
      { source: { caseStepId: "S0-3", method: "prisma" }, id: "db.target.email", title: "被管理成员 的邮箱 应为 \"orf-member-disable-forbidden-target-e2e@orf.local\"", object: "db.user", operator: "matches", params: { userIdFrom: "data.targetUserId", emailFrom: "data.targetEmail" } },
  ...
  S1: {
    description: "普通成员位于悬赏大厅，系统管理入口和成员停用入口不存在，被管理成员状态保持不变",
    assertions: [
      ...
      { source: { caseStepId: "S1-4", method: "playwright" }, id: "page.disable_action.absent", title: "成员停用入口 应不可见", object: "page.member_management", operator: "disable_action_absent" },
      { source: { caseStepId: "S1-5", method: "prisma" }, id: "db.target.email_unchanged", title: "被管理成员 的邮箱 应仍为 \"orf-member-disable-forbidden-target-e2e@orf.local\"", object: "db.user", operator: "matches", params: { userIdFrom: "data.targetUserId", emailFrom: "data.targetEmail" } },
      ...
      { source: { caseStepId: "S1-8", method: "api" }, id: "session.member.still_authenticated", title: "当前会话 应仍为 邮箱为 `orf-member-disable-forbidden-e2e@orf.local`、角色为 `member`、状态为 `active` 的已登录会话", object: "auth.session", operator: "authenticated", params: { emailFrom: "data.memberEmail", roleFrom: "data.memberRole", status: "active" } },
    ],
  },
  ...
  Clean: {
    ...
    steps: [
      ...
      { source: { caseStepId: "Clean-6", method: "api" }, id: "ory.member_identity.delete", title: "删除邮箱为 `orf-member-disable-forbidden-e2e@orf.local` 的普通成员认证身份", object: "ory.identity", operator: "delete_by_email", params: { emailFrom: "data.memberEmail" } },
      ...
      { source: { caseStepId: "Clean-8", method: "prisma" }, id: "db.member.delete", title: "删除邮箱为 `orf-member-disable-forbidden-e2e@orf.local` 的普通成员用户", object: "db.user", operator: "delete", params: { emailFrom: "data.memberEmail" } },
      ...
      { source: { caseStepId: "Clean-10", method: "api" }, id: "ory.member_identity.absent", title: "邮箱为 `orf-member-disable-forbidden-e2e@orf.local` 的普通成员认证身份 应不存在", object: "ory.identity", operator: "absent", params: { emailFrom: "data.memberEmail" } },
      { source: { caseStepId: "Clean-11", method: "prisma" }, id: "db.member.absent", title: "邮箱为 `orf-member-disable-forbidden-e2e@orf.local` 的普通成员用户 应不存在", object: "db.user", operator: "absent", params: { emailFrom: "data.memberEmail" } },
    ],
  },
