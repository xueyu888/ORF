...
import { adminDeleteMemberForbiddenCase } from "../admin-delete-member-forbidden.case";

test.describe("管理员删除成员反向测试用例", () => {
  test(adminDeleteMemberForbiddenCase.title, async ({ context, page }, testInfo) => {
    const ctx: MemberManagementForbiddenTestContext = { context, page };

    await runStateCase(adminDeleteMemberForbiddenCase, ctx, {
