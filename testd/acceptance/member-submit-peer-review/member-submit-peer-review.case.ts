import { STATE_CASE_MODEL, type StateCaseSpec } from "../../_framework/types";
import type { MemberSubmitPeerReviewCaseData } from "./_support/member-submit-peer-review.context";

export const memberSubmitPeerReviewCase = {
  id: "acceptance.member-submit-peer-review",
  title: "成员提交匿名互评",
  model: STATE_CASE_MODEL,
  tags: ["acceptance", "peer-review", "member", "happy-path"],

  data: {
    email: "orf-member-submit-peer-review-e2e@orf.local",
    password: "OrfMemberSubmitPeerReviewE2E!2026",
    name: "ORF Member Submit Peer Review E2E",
    peerEmail: "orf-peer-submit-peer-review-e2e@orf.local",
    peerPassword: "OrfPeerSubmitPeerReviewE2E!2026",
    peerName: "ORF Peer Submit Peer Review E2E",
    role: "member",
    objectiveId: "obj-testd-member-submit-peer-review",
    objectiveTitle: "E2E-MEMBER-SUBMIT-PEER-REVIEW: 目标前置",
    lootBody: "E2E-PEER-LOOT-BODY: 匿名互评前置战利品",
    reviewerRatio: "50",
    peerRatio: "50",
  },

  B: {
    description: "系统服务可用，浏览器处于未登录基准状态",
    assertions: [
      {
        source: { caseStepId: "B-1", method: "api" },
        id: "frontend.ready",
        title: "前端服务 应可用",
        object: "frontend.service",
        operator: "available",
      },
      {
        source: { caseStepId: "B-2", method: "api" },
        id: "backend.ready",
        title: "后端服务 应可用",
        object: "api.health",
        operator: "ok",
      },
      {
        source: { caseStepId: "B-3", method: "api" },
        id: "frontend.login_entry.accessible",
        title: "前端登录页入口 应可访问",
        object: "frontend.login_entry",
        operator: "accessible",
      },
      {
        source: { caseStepId: "B-4", method: "api" },
        id: "session.endpoint.accessible",
        title: "当前会话查询能力 应可用",
        object: "auth.session",
        operator: "accessible",
      },
      {
        source: { caseStepId: "B-5", method: "prisma" },
        id: "db.ready",
        title: "ORF 数据库 应可连接",
        object: "db",
        operator: "ready",
      },
      {
        source: { caseStepId: "B-6", method: "prisma" },
        id: "db.schema.current",
        title: "ORF 数据库 schema 应为 当前测试版本",
        object: "db.schema",
        operator: "current",
      },
      {
        source: { caseStepId: "B-7", method: "api" },
        id: "ory.admin_public.ready",
        title: "Ory/Kratos 认证服务的管理和公共访问能力 应可用",
        object: "ory.admin_public",
        operator: "ready",
      },
      {
        source: { caseStepId: "B-8", method: "api" },
        id: "session.unauthenticated",
        title: "当前会话 应为 未登录",
        object: "auth.session",
        operator: "unauthenticated",
      },
      {
        source: { caseStepId: "B-9", method: "playwright" },
        id: "cookie.absent",
        title: "当前浏览器 应不存在 Ory 登录会话 cookie",
        object: "browser.cookie",
        operator: "absent",
      },
      {
        source: { caseStepId: "B-10", method: "playwright" },
        id: "storage.empty",
        title: "当前浏览器 应不保留本地登录态",
        object: "browser.auth_storage",
        operator: "empty",
      },
    ],
  },

  Setup: {
    description: "准备普通成员账号、本用例独占 accepted 目标和前置战利品，登录普通成员并进入战利品页面",
    steps: [
      {
        source: { caseStepId: "Setup-1", method: "prisma" },
        id: "db.peer_review.delete_residue",
        title: "删除可能残留的本用例普通成员服务端原始匿名互评记录",
        object: "db.peer_review",
        operator: "delete",
        params: { reviewerFrom: "data.name" },
      },
      {
        source: { caseStepId: "Setup-2", method: "prisma" },
        id: "db.peer_review_loot.delete_residue",
        title: "删除可能残留的内容为 `E2E-PEER-LOOT-BODY: 匿名互评前置战利品` 的测试战利品",
        object: "db.peer_review_loot",
        operator: "delete",
        params: { bodyFrom: "data.lootBody" },
      },
      {
        source: { caseStepId: "Setup-3", method: "prisma" },
        id: "db.objective.delete_residue",
        title: "删除可能残留的标题为 `E2E-MEMBER-SUBMIT-PEER-REVIEW: 目标前置` 的本用例目标及其派生数据",
        object: "db.objective",
        operator: "delete_by_title",
        params: { titleFrom: "data.objectiveTitle" },
      },
      {
        source: { caseStepId: "Setup-4", method: "api" },
        id: "ory.member_identity.upsert",
        title: "准备邮箱为 `orf-member-submit-peer-review-e2e@orf.local`、使用固定测试密码的普通成员登录身份",
        object: "ory.identity",
        operator: "upsert_password",
        params: {
          emailFrom: "data.email",
          nameFrom: "data.name",
          passwordFrom: "data.password",
          saveAs: "memberIdentity",
        },
      },
      {
        source: { caseStepId: "Setup-5", method: "prisma" },
        id: "db.member.upsert",
        title: "准备邮箱为 `orf-member-submit-peer-review-e2e@orf.local`、角色为 `member`、状态为 `active` 的普通成员用户和默认团队成员关系",
        object: "db.user",
        operator: "upsert",
        params: {
          emailFrom: "data.email",
          nameFrom: "data.name",
          roleFrom: "data.role",
          status: "active",
          identityIdFrom: "runtime.memberIdentity.id",
          saveAs: "memberUser",
        },
      },
      {
        source: { caseStepId: "Setup-6", method: "api" },
        id: "ory.peer_identity.upsert",
        title: "准备邮箱为 `orf-peer-submit-peer-review-e2e@orf.local`、使用固定测试密码的协作成员登录身份",
        object: "ory.identity",
        operator: "upsert_password",
        params: {
          emailFrom: "data.peerEmail",
          nameFrom: "data.peerName",
          passwordFrom: "data.peerPassword",
          saveAs: "peerIdentity",
        },
      },
      {
        source: { caseStepId: "Setup-7", method: "prisma" },
        id: "db.peer_member.upsert",
        title: "准备邮箱为 `orf-peer-submit-peer-review-e2e@orf.local`、角色为 `member`、状态为 `active` 的协作成员用户和默认团队成员关系",
        object: "db.user",
        operator: "upsert",
        params: {
          emailFrom: "data.peerEmail",
          nameFrom: "data.peerName",
          roleFrom: "data.role",
          status: "active",
          identityIdFrom: "runtime.peerIdentity.id",
          saveAs: "peerUser",
        },
      },
      {
        source: { caseStepId: "Setup-8", method: "prisma" },
        id: "db.objective.upsert_peer_review_target",
        title: "创建 本用例目标，标题为 `E2E-MEMBER-SUBMIT-PEER-REVIEW: 目标前置`、流转状态为 `accepted`",
        object: "db.objective",
        operator: "upsert",
        params: {
          idFrom: "data.objectiveId",
          titleFrom: "data.objectiveTitle",
          teamIdFrom: "runtime.memberUser.teamId",
          stage: "goalFrozen",
          flowStatus: "accepted",
          status: "Draft",
          saveAs: "fixtureObjective",
        },
      },
      {
        source: { caseStepId: "Setup-9", method: "prisma" },
        id: "db.peer_review_target.from_objective",
        title: "记录 本用例目标",
        object: "db.peer_review_target",
        operator: "from_objective",
        params: { objectiveIdFrom: "runtime.fixtureObjective.id", saveAs: "peerReviewTarget" },
      },
      {
        source: { caseStepId: "Setup-10", method: "prisma" },
        id: "db.peer_review_target.add_reviewer_challenger",
        title: "设置 本用例目标的挑战者包含提交成员",
        object: "db.peer_review_target",
        operator: "add_challenger",
        params: { targetFrom: "runtime.peerReviewTarget", memberNameFrom: "data.name", memberUserIdFrom: "runtime.memberUser.userId" },
      },
      {
        source: { caseStepId: "Setup-11", method: "prisma" },
        id: "db.peer_review_target.add_peer_challenger",
        title: "设置 本用例目标的挑战者包含协作成员",
        object: "db.peer_review_target",
        operator: "add_challenger",
        params: { targetFrom: "runtime.peerReviewTarget", memberNameFrom: "data.peerName", memberUserIdFrom: "runtime.peerUser.userId" },
      },
      {
        source: { caseStepId: "Setup-12", method: "prisma" },
        id: "db.peer_review_target.accepted_for_review",
        title: "设置 本用例目标为已验收状态",
        object: "db.peer_review_target",
        operator: "accepted_for_review",
        params: { targetFrom: "runtime.peerReviewTarget" },
      },
      {
        source: { caseStepId: "Setup-13", method: "prisma" },
        id: "db.peer_review_loot.create",
        title: "创建 本用例目标的前置测试战利品，内容为 `E2E-PEER-LOOT-BODY: 匿名互评前置战利品`",
        object: "db.peer_review_loot",
        operator: "create",
        params: {
          targetFrom: "runtime.peerReviewTarget",
          bodyFrom: "data.lootBody",
          memberNameFrom: "data.name",
          saveAs: "peerReviewLoot",
        },
      },
      {
        source: { caseStepId: "Setup-14", method: "api" },
        id: "ory.member_sessions.revoke",
        title: "撤销普通成员登录身份可能残留的登录会话",
        object: "ory.sessions",
        operator: "revoke_by_email",
        params: { emailFrom: "data.email" },
      },
      {
        source: { caseStepId: "Setup-15", method: "api" },
        id: "ory.peer_sessions.revoke",
        title: "撤销协作成员登录身份可能残留的登录会话",
        object: "ory.sessions",
        operator: "revoke_by_email",
        params: { emailFrom: "data.peerEmail" },
      },
      {
        source: { caseStepId: "Setup-16", method: "playwright" },
        id: "browser.clear",
        title: "移除当前浏览器中的残留登录态",
        object: "browser",
        operator: "clear_state",
      },
      {
        source: { caseStepId: "Setup-17", method: "playwright" },
        id: "page.goto.auth",
        title: "打开 ORF 登录页",
        object: "page",
        operator: "goto",
        params: { path: "/auth" },
      },
      {
        source: { caseStepId: "Setup-18", method: "playwright" },
        id: "fill.email",
        title: "在邮箱输入框输入普通成员固定测试邮箱",
        object: "page",
        operator: "fill",
        params: { label: "Email", valueFrom: "data.email" },
      },
      {
        source: { caseStepId: "Setup-19", method: "playwright" },
        id: "fill.password",
        title: "在密码输入框输入普通成员固定测试密码",
        object: "page",
        operator: "fill",
        params: { label: "Password", exact: true, valueFrom: "data.password" },
      },
      {
        source: { caseStepId: "Setup-20", method: "playwright" },
        id: "click.sign_in",
        title: "点击 \"Sign In\" 登录操作",
        object: "page",
        operator: "click",
        params: { role: "button", name: "Sign In" },
      },
      {
        source: { caseStepId: "Setup-21", method: "api" },
        id: "session.member.authenticated",
        title: "当前会话 应为 普通成员的已登录会话",
        object: "auth.session",
        operator: "authenticated",
        params: { emailFrom: "data.email", roleFrom: "data.role", status: "active" },
      },
      {
        source: { caseStepId: "Setup-22", method: "playwright" },
        id: "page.goto.loot",
        title: "打开 任务模块下的本用例目标战利品页面",
        object: "page.peer_review",
        operator: "goto",
        params: { targetFrom: "runtime.peerReviewTarget" },
      },
    ],
  },

  S0: {
    description: "普通成员已登录并位于本用例目标战利品页面，目标可提交匿名互评",
    assertions: [
      {
        source: { caseStepId: "S0-1", method: "api" },
        id: "session.member.authenticated",
        title: "当前会话 应为 邮箱为 `orf-member-submit-peer-review-e2e@orf.local`、角色为 `member`、状态为 `active` 的已登录会话",
        object: "auth.session",
        operator: "authenticated",
        params: { emailFrom: "data.email", roleFrom: "data.role", status: "active" },
      },
      {
        source: { caseStepId: "S0-2", method: "playwright" },
        id: "url.loot",
        title: "当前页面 应为 任务模块下的本用例目标战利品页面",
        object: "page.url",
        operator: "match",
        params: { pattern: "/tasks/objectives/.+/loot$" },
      },
      {
        source: { caseStepId: "S0-3", method: "playwright" },
        id: "peer_review_form.visible",
        title: "匿名互评表单 应可见",
        object: "page.peer_review_form",
        operator: "visible",
      },
      {
        source: { caseStepId: "S0-4", method: "prisma" },
        id: "db.peer_review_target.accepted",
        title: "本用例目标 应为 流转状态 `accepted`、验收结果为 `completed` 且战利品提交时间已存在",
        object: "db.peer_review_target",
        operator: "accepted",
        params: { targetFrom: "runtime.peerReviewTarget" },
      },
      {
        source: { caseStepId: "S0-5", method: "prisma" },
        id: "db.peer_review_target.reviewer_challenger_present",
        title: "本用例目标的挑战者 应包含 提交成员",
        object: "db.peer_review_target",
        operator: "challenger_present",
        params: { targetFrom: "runtime.peerReviewTarget", memberNameFrom: "data.name", memberUserIdFrom: "runtime.memberUser.userId" },
      },
      {
        source: { caseStepId: "S0-6", method: "prisma" },
        id: "db.peer_review_target.peer_challenger_present",
        title: "本用例目标的挑战者 应包含 协作成员",
        object: "db.peer_review_target",
        operator: "challenger_present",
        params: { targetFrom: "runtime.peerReviewTarget", memberNameFrom: "data.peerName", memberUserIdFrom: "runtime.peerUser.userId" },
      },
      {
        source: { caseStepId: "S0-7", method: "prisma" },
        id: "db.peer_review_loot.present",
        title: "本用例目标 应存在 内容为 `E2E-PEER-LOOT-BODY: 匿名互评前置战利品` 的测试战利品",
        object: "db.peer_review_loot",
        operator: "present",
        params: { targetFrom: "runtime.peerReviewTarget", lootFrom: "runtime.peerReviewLoot" },
      },
      {
        source: { caseStepId: "S0-8", method: "prisma" },
        id: "db.peer_review.absent",
        title: "本用例目标 应不存在 提交成员提交的服务端原始匿名互评",
        object: "db.peer_review",
        operator: "absent",
        params: { targetFrom: "runtime.peerReviewTarget", reviewerFrom: "data.name" },
      },
    ],
  },

  Action: {
    description: "普通成员通过页面提交测试匿名互评",
    steps: [
      {
        source: { caseStepId: "Action-1", method: "playwright" },
        id: "fill.reviewer_ratio",
        title: "在提交成员贡献百分比输入框 输入 `50`",
        object: "page",
        operator: "fill",
        params: { labelFrom: "data.name", valueFrom: "data.reviewerRatio" },
      },
      {
        source: { caseStepId: "Action-2", method: "playwright" },
        id: "fill.peer_ratio",
        title: "在协作成员贡献百分比输入框 输入 `50`",
        object: "page",
        operator: "fill",
        params: { labelFrom: "data.peerName", valueFrom: "data.peerRatio" },
      },
      {
        source: { caseStepId: "Action-3", method: "playwright" },
        id: "submit.peer_review_form",
        title: "点击 \"提交匿名互评\" 提交匿名互评操作",
        object: "page.peer_review_form",
        operator: "submit",
        params: { targetFrom: "runtime.peerReviewTarget", saveAs: "submitPeerReviewResponse" },
      },
      {
        source: { caseStepId: "Action-4", method: "api" },
        id: "submit_peer_review_response.record",
        title: "记录 ORF 同源代理转发到本地结算服务的加密匿名互评提交",
        object: "api.peer_review_submit_response",
        operator: "record_review",
        params: { responseFrom: "runtime.submitPeerReviewResponse", saveAs: "submittedPeerReview" },
      },
    ],
  },

  S1: {
    description: "本地结算服务已收到加密匿名互评，ORF 服务端未保存原始互评数据，普通成员仍保持登录",
    assertions: [
      {
        source: { caseStepId: "S1-1", method: "api" },
        id: "submitted_peer_review.sent_to_local_service",
        title: "提交后的匿名互评请求 应经 ORF 同源代理发送到 本地结算服务",
        object: "api.peer_review_submit_response",
        operator: "sent_to_local_service",
        params: { reviewFrom: "runtime.submittedPeerReview" },
      },
      {
        source: { caseStepId: "S1-2", method: "api" },
        id: "submitted_peer_review.accepted",
        title: "本地结算服务 应接受 加密匿名互评提交",
        object: "api.peer_review_submit_response",
        operator: "accepted",
        params: { reviewFrom: "runtime.submittedPeerReview" },
      },
      {
        source: { caseStepId: "S1-3", method: "api" },
        id: "submitted_peer_review.ciphertext",
        title: "提交后的匿名互评请求 应包含 加密正文",
        object: "api.peer_review_submit_response",
        operator: "encrypted_field",
        params: { reviewFrom: "runtime.submittedPeerReview", field: "ciphertext" },
      },
      {
        source: { caseStepId: "S1-4", method: "api" },
        id: "submitted_peer_review.encrypted_key",
        title: "提交后的匿名互评请求 应包含 加密数据密钥",
        object: "api.peer_review_submit_response",
        operator: "encrypted_field",
        params: { reviewFrom: "runtime.submittedPeerReview", field: "encryptedKey" },
      },
      {
        source: { caseStepId: "S1-5", method: "prisma" },
        id: "db.peer_review.raw_absent_after_submit",
        title: "ORF 数据库 应不存在 提交成员提交的原始匿名互评",
        object: "db.peer_review",
        operator: "absent",
        params: { targetFrom: "runtime.peerReviewTarget", reviewerFrom: "data.name" },
      },
      {
        source: { caseStepId: "S1-6", method: "api" },
        id: "submitted_peer_review.no_reviewer_plaintext",
        title: "提交后的匿名互评加密信封 应不包含 提交成员明文",
        object: "api.peer_review_submit_response",
        operator: "no_plaintext",
        params: { reviewFrom: "runtime.submittedPeerReview", textFrom: "data.name" },
      },
      {
        source: { caseStepId: "S1-7", method: "api" },
        id: "submitted_peer_review.no_peer_plaintext",
        title: "提交后的匿名互评加密信封 应不包含 协作成员明文",
        object: "api.peer_review_submit_response",
        operator: "no_plaintext",
        params: { reviewFrom: "runtime.submittedPeerReview", textFrom: "data.peerName" },
      },
      {
        source: { caseStepId: "S1-8", method: "api" },
        id: "session.member.still_authenticated",
        title: "当前会话 应仍为 普通成员的已登录会话",
        object: "auth.session",
        operator: "authenticated",
        params: { emailFrom: "data.email", roleFrom: "data.role", status: "active" },
      },
    ],
  },

  Clean: {
    description: "删除本用例可能残留的服务端原始互评、战利品、目标、普通成员账号和页面会话状态",
    steps: [
      {
        source: { caseStepId: "Clean-1", method: "prisma" },
        id: "db.peer_review.delete",
        title: "删除 本用例可能残留的服务端原始匿名互评",
        object: "db.peer_review",
        operator: "delete",
        params: { targetFrom: "runtime.peerReviewTarget", reviewerFrom: "data.name", reviewFrom: "runtime.submittedPeerReview" },
      },
      {
        source: { caseStepId: "Clean-2", method: "prisma" },
        id: "db.peer_review_loot.delete",
        title: "删除 本用例创建的测试战利品",
        object: "db.peer_review_loot",
        operator: "delete",
        params: { bodyFrom: "data.lootBody", lootFrom: "runtime.peerReviewLoot" },
      },
      {
        source: { caseStepId: "Clean-3", method: "prisma" },
        id: "db.objective.delete",
        title: "删除 本用例独占目标及其派生数据",
        object: "db.objective",
        operator: "delete",
        params: { idFrom: "data.objectiveId", titleFrom: "data.objectiveTitle" },
      },
      {
        source: { caseStepId: "Clean-4", method: "api" },
        id: "auth.logout",
        title: "注销当前登录会话",
        object: "auth",
        operator: "logout",
      },
      {
        source: { caseStepId: "Clean-5", method: "playwright" },
        id: "page.runtime.stop",
        title: "离开当前 ORF 前端页面",
        object: "page.runtime",
        operator: "stop",
      },
      {
        source: { caseStepId: "Clean-6", method: "playwright" },
        id: "browser.clear",
        title: "移除当前浏览器中的残留登录态",
        object: "browser",
        operator: "clear_state",
      },
      {
        source: { caseStepId: "Clean-7", method: "api" },
        id: "ory.member_sessions.revoke",
        title: "撤销普通成员登录身份的残留登录会话",
        object: "ory.sessions",
        operator: "revoke_by_email",
        params: { emailFrom: "data.email" },
      },
      {
        source: { caseStepId: "Clean-8", method: "api" },
        id: "ory.peer_sessions.revoke",
        title: "撤销协作成员登录身份的残留登录会话",
        object: "ory.sessions",
        operator: "revoke_by_email",
        params: { emailFrom: "data.peerEmail" },
      },
      {
        source: { caseStepId: "Clean-9", method: "api" },
        id: "ory.member_identity.delete",
        title: "删除邮箱为 `orf-member-submit-peer-review-e2e@orf.local` 的普通成员登录身份",
        object: "ory.identity",
        operator: "delete_by_email",
        params: { emailFrom: "data.email" },
      },
      {
        source: { caseStepId: "Clean-10", method: "api" },
        id: "ory.peer_identity.delete",
        title: "删除邮箱为 `orf-peer-submit-peer-review-e2e@orf.local` 的协作成员登录身份",
        object: "ory.identity",
        operator: "delete_by_email",
        params: { emailFrom: "data.peerEmail" },
      },
      {
        source: { caseStepId: "Clean-11", method: "prisma" },
        id: "db.member.delete_memberships",
        title: "删除邮箱为 `orf-member-submit-peer-review-e2e@orf.local` 的普通成员默认团队成员关系",
        object: "db.user",
        operator: "delete_memberships",
        params: { emailFrom: "data.email" },
      },
      {
        source: { caseStepId: "Clean-12", method: "prisma" },
        id: "db.peer_member.delete_memberships",
        title: "删除邮箱为 `orf-peer-submit-peer-review-e2e@orf.local` 的协作成员默认团队成员关系",
        object: "db.user",
        operator: "delete_memberships",
        params: { emailFrom: "data.peerEmail" },
      },
      {
        source: { caseStepId: "Clean-13", method: "prisma" },
        id: "db.member.delete",
        title: "删除邮箱为 `orf-member-submit-peer-review-e2e@orf.local` 的普通成员用户",
        object: "db.user",
        operator: "delete",
        params: { emailFrom: "data.email" },
      },
      {
        source: { caseStepId: "Clean-14", method: "prisma" },
        id: "db.peer_member.delete",
        title: "删除邮箱为 `orf-peer-submit-peer-review-e2e@orf.local` 的协作成员用户",
        object: "db.user",
        operator: "delete",
        params: { emailFrom: "data.peerEmail" },
      },
      {
        source: { caseStepId: "Clean-15", method: "prisma" },
        id: "db.peer_review.absent",
        title: "应不存在 本用例提交成员提交的服务端原始匿名互评",
        object: "db.peer_review",
        operator: "absent",
        params: { targetFrom: "runtime.peerReviewTarget", reviewerFrom: "data.name" },
      },
      {
        source: { caseStepId: "Clean-16", method: "prisma" },
        id: "db.peer_review_loot.absent",
        title: "应不存在 内容为 `E2E-PEER-LOOT-BODY: 匿名互评前置战利品` 的测试战利品",
        object: "db.peer_review_loot",
        operator: "absent",
        params: { bodyFrom: "data.lootBody" },
      },
      {
        source: { caseStepId: "Clean-17", method: "prisma" },
        id: "db.objective.absent",
        title: "应不存在 标题为 `E2E-MEMBER-SUBMIT-PEER-REVIEW: 目标前置` 的本用例目标",
        object: "db.objective",
        operator: "absent",
        params: { idFrom: "data.objectiveId", titleFrom: "data.objectiveTitle" },
      },
      {
        source: { caseStepId: "Clean-18", method: "prisma" },
        id: "db.member.absent",
        title: "应不存在 邮箱为 `orf-member-submit-peer-review-e2e@orf.local` 的普通成员用户",
        object: "db.user",
        operator: "absent",
        params: { emailFrom: "data.email" },
      },
      {
        source: { caseStepId: "Clean-19", method: "prisma" },
        id: "db.peer_member.absent",
        title: "应不存在 邮箱为 `orf-peer-submit-peer-review-e2e@orf.local` 的协作成员用户",
        object: "db.user",
        operator: "absent",
        params: { emailFrom: "data.peerEmail" },
      },
    ],
  },
} satisfies StateCaseSpec<MemberSubmitPeerReviewCaseData>;
