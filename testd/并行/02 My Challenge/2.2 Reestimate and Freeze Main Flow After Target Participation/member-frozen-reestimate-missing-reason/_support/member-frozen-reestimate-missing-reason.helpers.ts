...
import type { ObjectiveAlignmentRequestKind, UncertaintyLevel } from "../../../../../../src/types/orf";
...
import type { FrozenObjectiveTargetData, MetricData, MyChallengesApiData } from "./member-frozen-reestimate-missing-reason.context";
...
export async function prepareCalibratedMetric(input: {
  ...
  const values = {
    ...
    detail: "TestD frozen reestimate missing reason metric fixture",
...
export async function clearFrozenReestimateReason(page: Page, targetTitle: string) {
...
export async function frozenReestimateReasonEmpty(page: Page, targetTitle: string) {
  const panel = objectivePanel(page, targetTitle);
  await expect(panel).toBeVisible();
  await expect(panel.getByLabel("重新重估理由", { exact: true })).toHaveValue("");
}

export async function frozenReestimateSubmitDisabled(page: Page, targetTitle: string) {
  const panel = objectivePanel(page, targetTitle);
  await expect(panel).toBeVisible();
  await expect(panel.getByRole("button", { name: "申请重新重估", exact: true })).toBeDisabled();
}
...
export async function myChallengesLacksOpenAlignmentRequest(page: Page, input: {
  targetTitle: string;
  kind: ObjectiveAlignmentRequestKind;
}) {
  const data = await readMyChallenges(page);
