...
import type {
  AdminDeleteTaskActiveCaseData,
  AdminDeleteTaskActiveFixture,
  AdminDeleteTaskActiveSubtaskFixture,
  AdminDeleteTaskActiveTarget,
  TestContext,
...
import {
  ...
  testTaskAbsent,
} from "./_support/admin-delete-task-active.helpers";
...
export const adminDeleteTaskActiveOperators = {
  "db.task_target": {
  ...
  "db.task": {
    create_fixture: async ({ params }) =>
      createFixtureTask({
        ...
        status: requiredString(params, "status") as AdminDeleteTaskActiveFixture["status"],
        priority: requiredString(params, "priority") as AdminDeleteTaskActiveFixture["priority"],
      }),
  ...
  "browser.dialog": {
    ...
    accept_delete_task: async ({ params }) => {
      const dialog = requiredDialog(params, "dialog");
...
} satisfies OperatorRegistry<TestContext, AdminDeleteTaskActiveCaseData>;
...
async function openTaskDeleteMenu(page: TestContext["page"], target: AdminDeleteTaskActiveTarget, task: AdminDeleteTaskActiveFixture) {
...
function requiredTaskTarget(params: StepParams, key: string): AdminDeleteTaskActiveTarget {
  const value = params[key];
  if (
    typeof value !== "object" ||
    value === null ||
    typeof (value as AdminDeleteTaskActiveTarget).objective !== "object" ||
    (value as AdminDeleteTaskActiveTarget).objective === null ||
    typeof (value as AdminDeleteTaskActiveTarget).objective.id !== "string" ||
    typeof (value as AdminDeleteTaskActiveTarget).objective.title !== "string" ||
    typeof (value as AdminDeleteTaskActiveTarget).objective.flowStatus !== "string"
  ) {
    throw new Error(`参数 ${key} 必须是管理员删除行动项用例目标`);
  }

  return value as AdminDeleteTaskActiveTarget;
}

function requiredTask(params: StepParams, key: string): AdminDeleteTaskActiveFixture {
  const value = params[key];
  if (
    typeof value !== "object" ||
    value === null ||
    typeof (value as AdminDeleteTaskActiveFixture).id !== "string" ||
    typeof (value as AdminDeleteTaskActiveFixture).title !== "string" ||
    typeof (value as AdminDeleteTaskActiveFixture).description !== "string" ||
    typeof (value as AdminDeleteTaskActiveFixture).assignee !== "string" ||
    typeof (value as AdminDeleteTaskActiveFixture).linkedObjectiveId !== "string" ||
    typeof (value as AdminDeleteTaskActiveFixture).status !== "string" ||
    typeof (value as AdminDeleteTaskActiveFixture).priority !== "string"
  ) {
    throw new Error(`参数 ${key} 必须是管理员删除行动项测试任务`);
  }

  return value as AdminDeleteTaskActiveFixture;
}

function optionalTask(params: StepParams, key: string): AdminDeleteTaskActiveFixture | null {
  const value = params[key];
...
function requiredSubtask(params: StepParams, key: string): AdminDeleteTaskActiveSubtaskFixture {
  const value = params[key];
  if (
    typeof value !== "object" ||
    value === null ||
    typeof (value as AdminDeleteTaskActiveSubtaskFixture).id !== "string" ||
    typeof (value as AdminDeleteTaskActiveSubtaskFixture).taskId !== "string" ||
    typeof (value as AdminDeleteTaskActiveSubtaskFixture).label !== "string" ||
    typeof (value as AdminDeleteTaskActiveSubtaskFixture).done !== "boolean"
  ) {
    throw new Error(`参数 ${key} 必须是管理员删除行动项测试子行动项`);
  }

  return value as AdminDeleteTaskActiveSubtaskFixture;
}
