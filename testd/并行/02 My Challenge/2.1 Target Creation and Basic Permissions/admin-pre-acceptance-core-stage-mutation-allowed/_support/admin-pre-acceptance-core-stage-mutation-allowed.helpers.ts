...
import type { ObjectiveRecord, ObjectiveStageTargetData, TestUserAccountRecord } from "./admin-pre-acceptance-core-stage-mutation-allowed.context";
...
export async function confirmNextObjectiveDelete(page: Page) {
  const queue = pendingDeletesByPage.get(page) ?? [];
