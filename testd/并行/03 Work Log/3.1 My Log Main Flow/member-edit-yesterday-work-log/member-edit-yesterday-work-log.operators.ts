...
import type {
  MemberEditYesterdayWorkLogCaseData,
  TestContext,
  WorkLogDateScope,
  WorkLogEntryFixture,
...
import {
  ...
  workLogViewTab,
  yesterdayWorkDate,
} from "./_support/member-edit-yesterday-work-log.helpers";

export const memberEditYesterdayWorkLogOperators: OperatorRegistry<TestContext, MemberEditYesterdayWorkLogCaseData> = {
  "page.auth": {
  ...
  "page.work_logs.edit_action": {
    visible: async ({ ctx, params }) => {
      await expect(workLogEditButton(ctx.page, requiredString(params, "bodyMarker"))).toBeVisible();
    },

    enabled: async ({ ctx, params }) => {
      await expect(workLogEditButton(ctx.page, requiredString(params, "bodyMarker"))).toBeEnabled();
    },

    click: async ({ ctx, params }) => {
      await workLogEditButton(ctx.page, requiredString(params, "bodyMarker")).click();
      await expect(updateWorkLogButton(ctx.page)).toBeVisible();
    },
  },

  "page.work_logs.update_action": {
    update: async ({ ctx }) => {
      await updateWorkLog(ctx.page);
    },
  },
