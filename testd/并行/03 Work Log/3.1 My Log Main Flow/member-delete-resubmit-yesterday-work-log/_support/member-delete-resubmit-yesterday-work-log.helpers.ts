...
} from "./member-delete-resubmit-yesterday-work-log.context";
...
export function workLogDeleteButton(page: Page, bodyMarker: string) {
...
export async function deleteWorkLog(page: Page, bodyMarker: string) {
  const responsePromise = page
    ...
    .then(async (response) => {
      if (!response.ok()) {
        throw new Error(`删除工作日志接口失败: ${response.status()} ${JSON.stringify(await readResponseBody(response))}`);
      }
      return response;
    });
