...
export const topbarNavCase = {
  ...
  S1: {
    ...
    assertions: [
      ...
      step("S1-6", "playwright", "topbar.minimize.visible", "顶部导航栏的最小化按钮 应可见", "topbar.window.minimize", "visible"),
      step("S1-7", "playwright", "topbar.minimize.enabled", "顶部导航栏的最小化按钮 应可点击", "topbar.window.minimize", "enabled"),
      step("S1-8", "playwright", "topbar.restore.visible", "顶部导航栏的还原窗口按钮 应可见", "topbar.window.restore", "visible"),
      step("S1-9", "playwright", "topbar.restore.enabled", "顶部导航栏的还原窗口按钮 应可点击", "topbar.window.restore", "enabled"),
      step("S1-10", "playwright", "topbar.close_to_tray.visible", "顶部导航栏的关闭到托盘按钮 应可见", "topbar.window.close_to_tray", "visible"),
      step("S1-11", "playwright", "topbar.close_to_tray.enabled", "顶部导航栏的关闭到托盘按钮 应可点击", "topbar.window.close_to_tray", "enabled"),
      step("S1-12", "api", "session.authenticated.after_observe", "当前会话 应仍为 邮箱为 `orf-default-page-nav-e2e@orf.local`、角色为 `member`、状态为 `active` 的已登录会话", "auth.session", "authenticated", { emailFrom: "data.email", roleFrom: "data.role", status: "active" }),
    ],
  },
