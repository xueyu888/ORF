...
export const sidebarNavCase = {
  ...
  Setup: {
    ...
    steps: [
      ...
      step("Setup-4", "api", "user.preferences.sidebar_collapsed.expanded", "设置默认系统页测试用户侧边栏默认状态为 展开", "user.preferences", "set_sidebar_collapsed_by_email", { emailFrom: "data.email", collapsed: false }),
      step("Setup-5", "api", "ory.sessions.revoke", "撤销邮箱为 `orf-default-page-sidebar-e2e@orf.local` 的默认系统页测试认证身份的残留登录会话", "ory.sessions", "revoke_by_email", { emailFrom: "data.email" }),
      ...
      step("Setup-8", "playwright", "fill.email", "在邮箱输入框输入 `orf-default-page-sidebar-e2e@orf.local`", "page", "fill", { label: "Email", valueFrom: "data.email" }),
      step("Setup-9", "playwright", "fill.password", "在密码输入框输入默认系统页测试固定密码", "page", "fill", { label: "Password", exact: true, valueFrom: "data.password" }),
      step("Setup-10", "playwright", "click.sign_in", "点击 \"Sign In\" 登录操作", "page.login_form", "submit", { saveAs: "loginResponse" }),
      step("Setup-11", "api", "session.authenticated", "当前会话 应为 邮箱为 `orf-default-page-sidebar-e2e@orf.local`、角色为 `member`、状态为 `active` 的已登录会话", "auth.session", "authenticated", { emailFrom: "data.email", roleFrom: "data.role", status: "active" }),
  ...
  S0: {
    ...
    assertions: [
      ...
      step("S0-5", "playwright", "sidebar.initial_expanded", "侧边栏初始状态 应处于展开状态", "page.sidebar", "collapsed", { expected: false }),
    ],
  },
  Action: {
    ...
    steps: [
      ...
      step("Action-2", "playwright", "sidebar.collapse", "收起 首页（默认系统页）侧边栏", "page.sidebar.toggle", "click", { name: "折叠侧边栏", emailFrom: "data.email", expectedCollapsed: true }),
      step("Action-3", "playwright", "sidebar.collapse.confirm", "侧边栏收起后 应处于收起状态", "page.sidebar", "collapsed", { expected: true }),
      step("Action-4", "playwright", "sidebar.expand", "展开 首页（默认系统页）侧边栏", "page.sidebar.toggle", "click", { name: "展开侧边栏", emailFrom: "data.email", expectedCollapsed: false }),
    ],
  },
  S1: {
  ...
  Clean: {
    ...
    steps: [
      ...
      step("Clean-7", "api", "user.preferences.sidebar_collapsed.reset", "恢复邮箱为 `orf-default-page-sidebar-e2e@orf.local` 的侧边栏默认状态为 系统默认", "user.preferences", "reset_sidebar_collapsed_by_email", { emailFrom: "data.email" }),
      step("Clean-8", "prisma", "db.user.memberships.delete", "删除邮箱为 `orf-default-page-sidebar-e2e@orf.local` 的默认系统页测试用户的默认团队成员关系", "db.user", "delete_memberships", { emailFrom: "data.email" }),
      step("Clean-9", "prisma", "db.user.delete", "删除邮箱为 `orf-default-page-sidebar-e2e@orf.local` 的默认系统页测试用户", "db.user", "delete", { emailFrom: "data.email" }),
      step("Clean-10", "api", "ory.identity.absent", "邮箱为 `orf-default-page-sidebar-e2e@orf.local` 的默认系统页测试认证身份 应不存在", "ory.identity", "absent", { emailFrom: "data.email" }),
      step("Clean-11", "prisma", "db.user.absent", "邮箱为 `orf-default-page-sidebar-e2e@orf.local` 的默认系统页测试用户 应不存在", "db.user", "absent", { emailFrom: "data.email" }),
    ],
  },
