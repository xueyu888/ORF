...
export const feedbackImageUploadCase = {
  ...
  data: {
    ...
    unsupportedFileName: "orf-feedback-invalid-upload.txt",
    uploadErrorMessage: "只能上传 PNG、JPEG、GIF 或 WebP 图片",
  },
  B: {
  ...
  Setup: {
    ...
    steps: [
      ...
      step("Setup-5", "mock", "mock.unsupported_file.prepare", "准备本用例不支持格式文件", "mock.feedback_file", "prepare_unsupported", { fileNameFrom: "data.unsupportedFileName", saveAs: "unsupportedFile" }),
  ...
  S0: {
    description: "新建反馈页面表单可见，反馈描述为空且添加图片操作可用",
    assertions: [
      ...
      step("S0-5", "playwright", "feedback_create.add_image.enabled", "新建反馈表单的 \"添加图片\" 操作 应可点击", "feedback_create.add_image", "enabled"),
      step("S0-6", "playwright", "feedback_create.upload_error.hidden", "新建反馈表单 应不显示 图片格式不支持提示", "feedback_create.upload_error", "hidden", { messageFrom: "data.uploadErrorMessage" }),
    ],
  },
  Action: {
    description: "通过添加图片入口依次选择有效图片和不支持格式文件",
    steps: [
      step("Action-1", "playwright", "feedback_create.add_image.choose_valid", "通过新建反馈表单的 \"添加图片\" 操作选择本用例有效图片文件", "feedback_create.add_image", "choose", { filePathFrom: "runtime.validImage.path", fileNameFrom: "data.validImageFileName" }),
      step("Action-2", "playwright", "feedback_create.add_image.choose_unsupported", "通过新建反馈表单的 \"添加图片\" 操作选择本用例不支持格式文件", "feedback_create.add_image", "choose", { filePathFrom: "runtime.unsupportedFile.path", fileNameFrom: "data.unsupportedFileName" }),
    ],
  },
  S1: {
    description: "有效图片被添加为反馈附件，不支持格式文件被拒绝并展示错误提示",
    assertions: [
      ...
      step("S1-3", "playwright", "feedback_create.upload_error.visible", "新建反馈表单 应显示 `只能上传 PNG、JPEG、GIF 或 WebP 图片`", "feedback_create.upload_error", "visible", { messageFrom: "data.uploadErrorMessage" }),
      step("S1-4", "playwright", "feedback_create.description_input.not_contains_unsupported", "新建反馈表单的反馈描述输入区域 应不包含 本用例不支持格式文件引用", "feedback_create.description_input", "not_contains_file_reference", { fileNameFrom: "data.unsupportedFileName" }),
